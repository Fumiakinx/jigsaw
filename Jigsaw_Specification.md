...
## 7. セーブ・ロード機能 ＆ オートセーブ仕様 (Save/Load & Autosave Specifications)

### 7.1. セーブ・ロードの基本設計
パズルのプレイ状態をブラウザ側のLocalStorageに保存し、いつでもその状態から再開できる機能です。セーブデータには、各ピースの正確な座標、回転角度、およびピース同士のグループ結合関係（親子ツリー構造）が完全に保持されます。セーブ用のスロットは合計3つ提供されます。
...
* **ピクセル読み込み**: 強制描画された直後のバックバッファからピクセル情報を読み込み、JPG形式にエンコードします。これにより、UIが一切写り込まない、純粋なゲーム進捗のみの美しいサムネイルが撮影されます。撮影完了後、UIは即座に再表示されます。
...
### 7.4. オートセーブ機能の非表示化と今後の課題（バックログ）
* **現状と問題点**: ピース結合時等に自動的にセーブとサムネイル撮影を行う「オートセーブ」機能は、WebGL環境下においてCPU/GPUのボトルネック（特にピクセルの読み込みとJPG画像エンコード時のメインスレッド占有）となり、プレイ中にフレームレートの大幅な低下（引っかかり感）を招くことが判明しました。ゲーム本来のスムーズな操作性と気持ちよさを最優先するため、本機能は現在システム的に停止され、プレイヤーUI（コントロールバー）のプルダウンメニューからも非表示に設定されています。
* **将来の課題（バックログ）**:
  * サムネイル撮影プロセスの軽量化（高解像度キャプチャを廃止し、簡易的な進捗率のグラフィック表示に代えるなど）。
  * ピース座標データ（軽量テキスト）のみを即座に非同期保存し、高負荷な画像処理を完全に分離する軽量オートセーブアルゴリズムの確立。
  * Web Worker等を利用した、JPGエンコード処理のマルチスレッド化検討。
