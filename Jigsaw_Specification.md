...
光源 (左上) ＼
            ＼      [ハイライト面 (左・上エッジ)]
              ▼    ━━━━━━━━━━━━━━━━━
                    ( 鋭い3乗 of 加算光: specularIntensity 1.6 )
...
### 4.1. スナップ判定とツリー結合
ドラッグが終了した瞬間（`OnMouseUp`）、周囲の隣接するべきピースとの距離を計測します。
...
## 7. セーブ・ロード機能 ＆ 将来のオートセーブ課題 (Save/Load Specifications & Future Autosave Backlog)

### 7.1. セーブ・ロードの基本設計 (LocalStorage & Base64暗号化)
パズルのプレイ状態をブラウザ側のLocalStorageに保存し、いつでもその状態から再開できる機能です。
* **セーブデータのスロット**: スロットは合計3つ（スロット1〜3）提供され、プレイヤー名ごとに紐づけて管理されます。
* **保存内容**: 各ピースの正確な座標（`px`, `py`, `rx`, `ry`）、回転角度（`rz`）、およびピース同士の親子結合関係（ツリー構造）が完全に保持されます。
* **暗号化（難読化）処理**: LocalStorageへの保存および読み込み時、データの耐改ざん性・安全性を向上させるため、セーブデータ全体を Base64形式にて暗号化（難読化）処理して保持します。
* **保存容量制限の解決**: ゲーム中の下絵画像が巨大なBase64文字列である場合、LocalStorageの容量制限（約5MB）に達して保存エラーが起きるのを防ぐため、保存時には下絵画像を軽量な画像IDに置き換え、ロード時にGoogle Apps Script (GAS) 経由で再取得・復元する高度な最適化を実装しています。

### 7.2. 高品質サムネイル撮影（スクリーンショット）仕様
セーブデータ選択時にパズルの進捗が一目で判別できるよう、セーブ実行時のゲーム画面スナップショットをサムネイル画像（Base64形式）として保存します。
* **UI非表示と強制描画**: セーブボタンが押された瞬間、一時停止UIやセーブスロット選択UIなどのすべてのUI要素を一時的に非表示にします。その後、メインカメラに対して描画コマンドを強制的に呼び出すことで、画面上にゲーム画面のみがレンダリングされた状態を作り出します。
* **ピクセル読み込みと超高速軽量化**: 強制描画された直後のバックバッファからピクセル情報を読み込み、JPG形式にエンコードします。読み込み容量削減のため、さらに極小サムネイル（240x135）へと単純ピクセルサンプリングを用いて高速に縮小し、画質75%の軽量JPGとしてBase64化します。

### 7.3. ロード時のポップアップブロック回避と独自モーダルUI
ブラウザのセキュリティ制限（ポップアップブロック）を回避し、かつ極めてプレミアムなユーザー体験を提供するためのロード画面フローです。
* **セキュリティ問題の回避**: 非同期処理やゲームエンジン内部からの自動的なリダイレクト・新規タブオープン処理は、モダンブラウザによって「ポップアップブロック」の対象となります。
* **パズル再開モーダルの流用**: ロード要求が発生した際、ブラウザの標準警告ダイアログを使用するのではなく、画像選択画面と同じデザインテーマを持つ「美しい白地の独自モーダルUI」を表示します。
* **明示的なクリックイベント**: ユーザーがモーダル内の「パズル再開！」ボタンを物理的にクリックすることで、ブラウザはこれを正当なユーザーアクションと判定し、ポップアップブロックを完璧に回避して安全にパズルの再開画面へと遷移します。

### 7.4. オートセーブ機能の廃止と今後の課題（バックログ）
ピース結合時等に自動的にセーブとサムネイル撮影を行う「オートセーブ」機能は、一旦実装が見送られ、将来的な技術的改善課題（バックログ）として位置付けられました。
* **廃止の経緯**: WebGL環境下において、キャプチャ用のピクセル読み込みとJPG画像エンコード時のメインスレッド占有が非常に大きなCPU/GPUボトルネックとなり、プレイ中のフレームレートの低下（プチフリーズや引っかかり感）を招くことが判明しました。ゲーム本来のスムーズな操作性と気持ちよさを最優先するため、本機能は現在システム的に停止され、プレイヤーUI（コントロールバー）上のプルダウンメニューからも**完全に削除**されています。
* **将来の課題（バックログ）**:
  1. **サムネイル撮影プロセスの超軽量化**: オートセーブ時の高解像度キャプチャを完全に廃止し、簡易的な進捗率（例: 「結合ピース数/総ピース数」）のグラフィック表示に代えるなどの軽量化アプローチ。
  2. **非同期軽量オートセーブ**: ピース座標データ（軽量テキスト）のみを即座に非同期保存し、高負荷な画像処理を完全に分離する軽量化アルゴリズムの確立。
  3. **マルチスレッドエンコード**: Web Worker等を利用した、JPGエンコード処理のマルチスレッド化および非同期処理化の検証。
  4. **通常セーブ（手動）時のサムネイル作成プロセスのさらなる軽量化・非同期化**: 手動による通常セーブ時であっても、画面キャプチャやJPGエンコードによって発生し得る一瞬の描画遅延や引っかかりを完全に排除するため、セーブ処理全体のバックグラウンド非同期処理化や負荷分散を検討・課題化する。

---

本仕様書は、Jigsawプロジェクト of 「美しさ」「気持ちよさ」「安定性」を永続的に保証するためのバイブルです。
